...
export const handleSessionImageDeleteAction = async (args: ActionFunctionArgs) =>
    args.request.json().then((sessionDeleteData) => SessionImageApiServices.delete({
            osIdentifier: args.params.os_id!,
            sessionIdentifier: args.params.session_id!,
            imageIdentifier: args.params.image_id!,
            isAvailable: true,
            isHeader: false
        }, sessionDeleteData.token).then(() => redirect(`/os/${args.params.os_id!}/s/_/${args.params.session_id!}/i`))
    )
