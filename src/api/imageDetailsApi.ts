...
export const ImageDetailsApiService = {
    loadDetails: (image: TransientImageType) =>
        axios.get(Endpoints.openSpaceImageDetails(image)).then(response => {
    ...
    editDetails: (image: TransientImageType, details: ImageDetailsType) => axios.put(Endpoints.openSpaceImageDetails(image), details)
        .then(response => {
            return redirect('../..')
        })
